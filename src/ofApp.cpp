...
void ofApp::setup(){
    ...
    loadImagesFromFolder("img1", images1);
    loadImagesFromFolder("img2", images2);
    loadImagesFromFolder("img3", images3);
...
void ofApp::update(){
    ...
    if (currentTime - lastFrameTime1 >= frameDuration1) {
        updateFrame(currentFrameIndex1, images1.size(), direction1);
            lastFrameTime1 = currentTime;
    }
    if (currentTime - lastFrameTime2 >= frameDuration2) {
        updateFrame(currentFrameIndex2, images2.size(), direction2);
            lastFrameTime2 = currentTime;
    }
    if (currentTime - lastFrameTime3>= frameDuration3) {
        updateFrame(currentFrameIndex3, images3.size(), direction3);
